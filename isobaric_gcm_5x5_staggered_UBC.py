...
Transforming to Lorenz-like differencing at top to handle upper boundary condition properly
"""
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            for k in range(nP):
                ...
                if k == 0:
                    adv_omegau = ((omega_n[i,j,k+1]*u_n[i,j,k+1]-0.0)/(dPtop+dP[0])
                               -u_n[i,j,k]*(omega_n[i,j,k+1]-0.0)/(dPtop+dP[0]))
                elif k == nP-1:
                    adv_omegau = (omega_n[i,j,k]*(us[i,j]-u_n[i,j,k-1])/((Ps_n[i,j]-P[nP-1])+dP[k-1]))
                else:
        ...
        for j in range(1,nphi-1):
            for k in range(nP):
                ...
                adv_vv = (((v_n[i,j,k]/(a))*((v_n[i,j+1,k]
                       -v_n[i,j-1,k])/(2*dphi))))
                if k == 0:
                    adv_omegav = ((omega_n[i,j,k+1]*v_n[i,j,k+1]-0.0)/(dPtop+dP[0])
                               -v_n[i,j,k]*(omega_n[i,j,k+1]-0.0)/(dPtop+dP[0]))
                elif k == nP-1:
                    adv_omegav = (omega_n[i,j,k]*(vs[i,j]-v_n[i,j,k-1])/((Ps_n[i,j]-P[nP-1])+dP[k-1]))
                else:
                    adv_omegav = (omega_n[i,j,k]*(v_n[i,j,k+1]-v_n[i,j,k-1])/(dP[k]+dP[k-1]))
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            for k in range(nP):
                ...
                if k == 0:
                    adv_omegaT = (T_n[i,j,k]/theta_n[i,j,k]*((omega_n[i,j,k+1]
                               *theta_n[i,j,k+1]-0.0)/(dPtop+dP[0])-theta_n[i,j,k]
                               *(omega_n[i,j,k+1]-0.0)/(dPtop+dP[0])))
                elif k == nP-1:
                    adv_omegaT = (((omega_n[i,j,k]*T_n[i,j,k])/theta_n[i,j,k])
                       *((theta_s[i,j]-theta_n[i,j,k-1])/((Ps_n[i,j]-P[nP-1])+dP[k-1])))
                else:
                    adv_omegaT = (((omega_n[i,j,k]*T_n[i,j,k])/theta_n[i,j,k])
                       *((theta_n[i,j,k+1]-theta_n[i,j,k-1])/(dP[k]+dP[k+1])))
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
             for k in range(nP):
                 ...
                 int_div_v = 0.0
                 if k == 0:
                     int_div_u += (u_n[i+1,j,k]-u_n[i-1,j,k])/(2*dlambda)*(dP_stag[0]+0.5*dPtop)
                     int_div_v += (cosphi[j+1]*v_n[i,j+1,k]-cosphi[j-1]*v_n[i,j-1,k])/(2*dphi)*(dP_stag[0]+0.5*dPtop)
        ...
        for j in range(1,nphi-1):
            omegas_n[i,j] = (omega_n[i,j,nP-1]+(-(1./(a*np.cos(phi[j])))
                          *(us[i+1,j]-us[i-1,j])/(2*dlambda)+(1./(a*cosphi[j]))*(cosphi[j+1]*vs[i,j+1]
                          -cosphi[j-1]*vs[i,j-1])/(2*dphi))*(Ps_n[i,j]-(P[nP-1]+0.5*dP[nP-2])))
    return omegas_n
...
@jit
def filter_for_vertical_mixing(psi,dummy_psi,psi_s,tau):
    psi_filtered = psi.copy()
#    tau =16.
...
P = np.array([1,20,50,100,200,300,400,500,600,700,800,900],dtype=float)*100 #Pa
Ptop = 0.5*100 #Pa
dPtop = 0.5*100 #Pa
dP = np.array([19,30,50,100,100,100,100,100,100,100,100],dtype=float)*100 #Pa
dP_stag = np.array([9.5,15,50,100,100,100,100,100,100,100,100,100],dtype=float)*100 #Pa
Psponge = 10001
...
#        #Ps_f = second_order_shapiro_filter_2D(Ps_f)
    ...
    u_f = filter_for_vertical_mixing(u_f,dummy_psi,us,90)
    v_f = fourth_order_shapiro_filter(v_f)
    v_f = filter_for_vertical_mixing(v_f,dummy_psi,vs,90)
    T_f = fourth_order_shapiro_filter(T_f)
    T_f = filter_for_vertical_mixing(T_f,dummy_psi,Ts,90)
