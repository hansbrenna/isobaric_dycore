...
Transforming to Lorenz grid to handle upper boundary condition properly. u,v,T,Q on half
-levels, z,omega on full levels. Need z on half levels as well. Assuming T constant i 
layer, z at half level is mean of above and below, same for omega. Requires change to 
1st order differencing in p
"""
...
@jit
def prognostic_u(u_f, u_n, u_p, v_n, us, ubarp, omega_n, zbarp, Ps_n, Fl):
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            for k in range(nP):
                ...
                adv_uu = (
                    (u_n[i, j, k] / (a * cosphi[j]))
                    * ((u_n[i + 1, j, k] - u_n[i - 1, j, k]) / (2 * dlambda))
                )
                # advection of u in v-direction
                adv_vu = (
                    v_n[i, j, k]
                    / (a)
                    * (u_n[i, j + 1, k] - u_n[i, j - 1, k])
                    / (2 * dphi)
                )
                # advection of u in omega-direction
                if k == nP - 1:
                    adv_omegau = (
                        (omegas_n[i, j] * us[i, j] - omega_n[i, j, k] * ubarp[i, j, k])
                        / (Ps_n[i, j] - P[k])
                        - (
                            u_n[i, j, k]
                            * (omegas_n[i, j] - omega_n[i, j, k])
                            / (Ps_n[i, j, k] - P[k])
                        )
                    )
                else:
                    adv_omegau = (
                        ...
                        - (
                            u_n[i, j, k]
                            * (omega_n[i, j, k + 1] - omega_n[i, j, k])
                            / (dP[k])
                        )
                    )
                    # adv_omegau = (omega_n[i,j,k]*(u_n[i,j,k+1]-u_n[i,j,k-1])/(dP[k]+dP[k-1]))
                # coriolis term
...
@jit
def prognostic_v(v_f, v_n, v_p, u_n, vs, vbarp, omega_n, zbarp, Ps_n, Fphi):
    for i in range(nlambda):
        if i == nlambda - 1:
            i = -1
        for j in range(1, nphi - 1):
            for k in range(nP):
                ...
                adv_uv = (
                    (u_n[i, j, k] / (a * np.cos(phi[j])))
                    * ((v_n[i + 1, j, k] - v_n[i - 1, j, k]) / (2 * dlambda))
                )
                # advection of v in v-direction
                adv_vv = (
                    (
                        (v_n[i, j, k] / (a))
                        * ((v_n[i, j + 1, k] - v_n[i, j - 1, k]) / (2 * dphi))
                    )
                )
                if k == nP - 1:
                    adv_omegav = (
                        (omegas_n[i, j] * vs[i, j] - omega_n[i, j, k] * vbarp[i, j, k])
                        / (Ps_n[i, j] - P[k])
                        - (
                            v_n[i, j, k]
                            * (omegas_n[i, j] - omega_n[i, j, k])
                ...
                gdz_v = g / a * (zbarp[i, j + 1, k] - zbarp[i, j - 1, k]) / (2 * dphi)
...
# changing prognostic_T
@jit
def prognostic_T(
    T_f, T_n, T_p, u_n, v_n, omega_n, theta_n, thetabarp, Ps_n, theta_s, Q_n
):
    for i in range(nlambda):
        if i == nlambda - 1:
            i = -1
        for j in range(1, nphi - 1):
            for k in range(nP):
                adv_uT = (
                    (u_n[i, j, k] / (a * np.cos(phi[j])))
                    * ((T_n[i + 1, j, k] - T_n[i - 1, j, k]) / (2 * dlambda))
                )
                adv_vT = (v_n[i, j, k] / (a)) * (
                    (T_n[i, j + 1, k] - T_n[i, j - 1, k]) / (2 * dphi)
                )
                if k == nP - 1:
                    adv_omegaT = (
                        ((T_n[i, j, k]) / theta_n[i, j, k])
                        * (
                            (
                                omegas_n[i, j]
                                * theta_s[i, j]
                ...
                T_f[i, j, k] = (
                    ...
                    + 2
                    * dt
                    * (-(adv_uT + adv_vT + adv_omegaT) + Q_n[i, j, k])
                )
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            adv_P = (
                (us[i, j])
                / (a * np.cos(phi[j]))
                * (Ps_n[i + 1, j] - Ps_n[i - 1, j])
                / (2 * dlambda)
                + (vs[i, j] / (a))
                * (Ps_n[i, j + 1] - Ps_n[i, j - 1])
                / (2 * dphi)
            )
            ...
            for k in range(nP):
                ...
                if k == 0:
                    ...
                    for m in range(0, k + 1):
                        int_div_u += (u_n[i + 1, j, m] - u_n[i - 1, j, m]) / (
                            2 * dlambda
                        ) * dP[
                            k
                        ]
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            omegas_n[i, j] = (
                ...
                * (Ps_n[i, j] - (P[nP - 1] + 0.5 * dP[nP - 2]))
            )
    return omegas_n


@jit
def diag_z(z_n, T_n, dlnP):
    for i in range(nlambda):
        if i == nlambda - 1:
            i = -1
        for j in range(1, nphi - 1):
...
def helper_psibarp(psi):
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            for k in range(1, nP):
                psibarp[i, j, k] = 0.5 * (psi[i, j, k + 1] + psi[i, j, k])

            psibarp[i, j, 0] = psi[i, j, 0]
    return psibarp
...
def diag_surface_wind(us, vs, Vs, u_n, v_n, Ps_p, A):
    ...
    A = (
        kv_surf_w / (kv_surf_w + cv * Vs * (Ps_p - P[nP - 1]))
    )  # Vs and Ps used at previous time step
    us = A * u_n[:, :, nP - 1]
    vs = A * v_n[:, :, nP - 1]
...
def sponge_layer(sFl, sFphi, skv, u_n, v_n):
    for k in range(nP):
        if P[k] < Psponge:
            skv[:, :, k] = 5 * kf * ((Psponge - P[k]) / Psponge) ** 2
        else:
            skv[:, :, k] = 0.0
...
def first_order_shapiro_filter(psi):
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            for k in range(nP):
                psi_filtered_lambda[i, j, k] = (
                    0.25 * (psi[i - 1, j, k] + 2 * psi[i, j, k] + psi[i + 1, j, k])
                )
...
def first_order_shapiro_filter_2D(psi):
    ...
    for i in range(nlambda):
        if i == nlambda - 1:
            i = -1
        for j in range(1, nphi - 1):
            psi_filtered_lambda[i, j] = (
                0.25 * (psi[i - 1, j] + 2 * psi[i, j] + psi[i + 1, j])
            )
...
def alternative_second_order_shapiro_filter(psi, tau):
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            for k in range(nP):
                psi_filtered_lambda[i, j, k] = psi[i, j, k] + tau * (
                    (1. / 16.)
                    * (
                        -psi[i - 2, j, k]
                        + 4
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            ...
            for k in range(nP):
                if order == 2:
                    psi_filtered[i, j, k] = psi_filtered_lambda[i, j, k] + tau * (
                        (1. / 16.)
                        * (
                            -psi_filtered_lambda[i, j - 2, k]
                            + 4
                ...
                psi_filtered_lambda[i, j, k] = (
                    ...
                    * (
                        ...
                        * (psi[i - 2, j, k] + psi[i + 2, j, k])
                        + 8
                        * (psi[i - 3, j, k] + psi[i + 3, j, k])
                        - (psi[i - 4, j, k] + psi[i + 4, j, k])
                    )
                )
                    ...
                    psi_filtered[i, j, k] = (
                        ...
                        * (
                            ...
                            * (
                            ...
                            + 8
                            * (
                                psi_filtered_lambda[i, j - 3, k]
                                + psi_filtered_lambda[i, j + 3, k]
                            )
                            - (
                                psi_filtered_lambda[i, j - 4, k]
                                + psi_filtered_lambda[i, j + 4, k]
                            )
                        )
                    )
                    ...
                    psi_filtered[i, j] = (
                        ...
                        * (
                            ...
                            * (
                            ...
                            + 8
                            * (
                                psi_filtered_lambda[i, j - 3]
                                + psi_filtered_lambda[i, j + 3]
                            )
                            - (
                                psi_filtered_lambda[i, j - 4]
                                + psi_filtered_lambda[i, j + 4]
                            )
                        )
                    )
...
P = np.array(
    [1, 20, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900], dtype=float
) * 100  # Pa
dP = np.array(
    [19, 30, 50, 100, 100, 100, 100, 100, 100, 100, 100], dtype=float
) * 100  # Pa
dP_stag = np.array(
    [9.5, 15, 50, 100, 100, 100, 100, 100, 100, 100, 100, 100], dtype=float
) * 100  # Pa
Psponge = 21000
...
if init:
    ...
    T_n = T_eq + np.random.random(
        size=[nlambda, nphi, nP]
    ) / 1e3  # Random perturbations to break hemispheric and zonal symmetry
    T_p = T_eq
    Ts = T_eqs
...
while t < tstop:
    ...
    if n % 100 == 0:
        ...
        print(
            "Ps.mean: {}".format(
                np.average(
                    Ps_n[:, 1:nphi - 1], axis=1, weights=np.cos(phi[1:nphi - 1])
                ).mean()
            )
        )
        Ps_mean.append(
    ...
    # handle output
    u_h, v_h, Ps_h, T_h, omega_h, omegas_h, Q_h, theta_h = (
        add_fields_to_history(
            u_h,
            u_n,
            v_h,
            v_n,
