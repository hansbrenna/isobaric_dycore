...
Current issues: Does not conserve total mass. drift in mean surface pressure.
~2% over 10 days

Proposed solution: Add a very simple surface scheme for wind to avoid no-slip 
lower boundary.
"""
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            for k in range(nP):
                ...
                adv_uu = ((u_n[i,j,k]/(a*np.cos(phi[j])))*((u_n[i+1,j,k]
                      -u_n[i-1,j,k])/(2*dlambda)))
                #advection of u in v-direction
                adv_vu = (v_n[i,j,k]/a)*(u_n[i,j+1,k]-u_n[i,j-1,k])/(2*dphi)
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            for k in range(nP):
                ...
                adv_vv = (v_n[i,j,k]/a)*((v_n[i,j+1,k]-v_n[i,j-1,k])/(2*dphi))
                if k == 0:
                    adv_omegav = 0.0
                elif k == nP-1:
...
def prognostic_Ps(Ps_f,Ps_n,Ps_p,omegas_n,lnPs):
    for i in range(nlambda):
        if i == nlambda-1:
            i=-1
        for j in range(1,nphi-1):
            Ps_f[i,j] = Ps_p[i,j]+2*dt*(omegas_n[i,j])
    #pole handling
    Ps_f[:,0] = Ps_f[:,1].mean()
    ...
    if Ps_f.min() < 91000.:
        Ps_f[np.where(Ps_f < 91000.)] = 91000.
    return Ps_f,lnPs
        ...
        for j in range(1,nphi-1):
             for k in range(nP):
                 ...
                 if k == 0:
                     ...
                     for m in range(0,k):
                         int_div_u += (u_n[i+1,j,m]-u_n[i-1,j,m])/(2*dlambda)*dP
                         int_div_v += (v_n[i,j+1,m]-v_n[i,j-1,m])/(2*dphi)*dP
                     omega_n[i,j,k] = -(1./(a*np.cos(phi[j]))*int_div_u+1./a*int_div_v)
    return omega_n
...
def fourth_order_shapiro_filter(psi):
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            for k in range(nP):
                psi_filtered[i,j,k] = ((1./256)*(186.*psi[i,j,k]+56.*(psi[i-1,j,k]
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            ...
            for k in range(nP):  
                if order == 4:
                    psi_filtered[i,j,k] = ((1./256.)*(186.*psi[i,j,k]+56.*(psi[i,j-1,k]
                               +psi[i,j+1,k])-28*(psi[i,j-2,k]+psi[i,j+2,k])
                               +8*(psi[i,j-3,k]+psi[i,j+3,k])-(psi[i,j-4,k]+psi[i,j+4,k])))
                elif order == 2:
                    psi_filtered[i,j,k] = ((1./16.)*(-psi[i,j-2,k]+4*psi[i,j-1,k]
                               +10*psi[i,j,k]+4*psi[i,j+1,k]-psi[i,j+2,k])) 
                elif order == 1:
                    psi_filtered[i,j,k] = ((0.25)*(psi[i,j-1,k]+2*psi[i,j,k]+psi[i,j+1,k]))
                else:
                    print('something went wrong with phi shapiro filter order')
                    sys.exit()
    return psi_filtered
...
def fourth_order_shapiro_filter_2D(psi):
    ...
    for i in range(nlambda):
        if i == nlambda-1:
            ...
            i=-4
        for j in range(1,nphi-1):
            psi_filtered[i,j] = ((1./256)*(186.*psi[i,j]+56.*(psi[i-1,j]
    ...
    for i in range(nlambda):
        ...
        for j in range(1,nphi-1):
            ...
            for k in range(nP):  
                if order == 4:
                    psi_filtered[i,j] = ((1./256.)*(186.*psi[i,j]+56.*(psi[i,j-1]
                               +psi[i,j+1])-28*(psi[i,j-2]+psi[i,j+2])
                               +8*(psi[i,j-3]+psi[i,j+3])-(psi[i,j-4]+psi[i,j+4])))
                elif order == 2:
                    psi_filtered[i,j] = ((1./16.)*(-psi[i,j-2]+4*psi[i,j-1]
                               +10*psi[i,j]+4*psi[i,j+1]-psi[i,j+2])) 
                elif order == 1:
                    psi_filtered[i,j] = ((0.25)*(psi[i,j-1]+2*psi[i,j]+psi[i,j+1]))
                else:
                    print('something went wrong with phi shapiro filter order')
                    sys.exit()
    return psi_filtered
...
def compare_filters(psi):
    psif = fourth_order_shapiro_filter(psi)
    psiff = filter_4dx_components(psi)
    plt.plot(phi,psi[30,:,8],phi,psif[30,:,8],phi,psiff[30,:,8])
    plt.figure()
    plt.plot(lambd,psi[:,8,8],lambd,psif[:,8,8],lambd,psiff[:,8,8])
...
while t<tstop:
    ...
    #Filter velocity components for 4dy modes
    #u_f = filter_4dx_components(u_f)
    #v_f = filter_4dx_components(v_f)
    #T_f = filter_4dx_components(T_f)
    if n % 5 == 0:
    ...
    if n%10 == 0:
        ...
        print('Ps.mean: {}'.format(Ps_n.mean()))
