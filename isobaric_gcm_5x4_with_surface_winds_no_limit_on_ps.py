             ...
             for k in range(nP):
                 ...
                 if k == 0:
                     ...
                     for m in range(0,k):
                         ...
                         int_div_v += (v_n[i,j+1,m]-v_n[i,j-1,m])/(2*dphi)*dP_m
                     omega_n[i,j,k] = -(1./(a*np.cos(phi[j]))*int_div_u+1./a*int_div_v)
    return omega_n

@jit
def diag_omegas(omegas_n,omega_n,u_m,v_m):
    k = nP-1
    for i in range(nlambda):
        if i == nlambda-1:
            i=-1
        for j in range(1,nphi-1):
            omegas_n[i,j] = (omega_n[i,j,nP-1]+(-(1./(a*np.cos(phi[j])))
                          *(u_m[i+1,j,k]-u_m[i-1,j,k])/(2*dlambda)+(1./a)*(v_m[i,j+1,k]
                          -v_m[i,j-1,k])/(2*dphi))*(Ps_n[i,j]-90000))
    return omegas_n
...
u_m = np.zeros(shape=[nlambda,nphi,nP])
v_m = np.zeros(shape=[nlambda,nphi,nP])
...
while t<tstop:
    ...
    u_m = helper_Tm(u_n,u_n,us)
    v_m = helper_Tm(v_n,v_n,vs)
    omega_n = diag_omega(omega_n,u_m,v_m)
    omegas_n = diag_omegas(omegas_n,omega_n,u_m,v_m)
    T_m = helper_Tm(T_m,T_n,Ts)
