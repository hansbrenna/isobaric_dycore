    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            for k in range(nP):
                ...
                adv_vu = (v_n[i, j, k] / a) * (u_n[i, j + 1, k] - u_n[i, j - 1, k]) / (
                    2 * dphi
                )
        ...
        for j in range(1, nphi - 1):
            for k in range(nP):
                ...
                if k == 0:
                    ...
                    adv_omegaT = (
                        ((omega_n[i, j, k] * T_n[i, j, k]) / theta_n[i, j, k])
                        * (
                            (theta_s[i, j] - theta_n[i, j, k - 1])
                            / ((Ps_n[i, j] - P[nP - 1]) + dP)
                        )
                    )
                else:
            ...
            for k in range(nP):
                ...
                if k == 0:
                    ...
                    for m in range(0, k + 1):
                        ...
                        int_div_v += (v_n[i, j + 1, m] - v_n[i, j - 1, m]) / (
                            2 * dphi
                        ) * dP_m
                    omega_n[i, j, k] = -(
                        1. / (a * np.cos(phi[j])) * int_div_u + 1. / a * int_div_v
                    )
    return omega_n
    ...
    for i in range(nlambda):
        ...
        for j in range(1, nphi - 1):
            for k in range(nP):
                int_Tdlnp = 0.0
                for m in range(k, nP):
                    int_Tdlnp += T_m[i, j, m] * dlnP[
                        i, j, m
                    ]  # Tm_n is average temp of layer between two levels.
