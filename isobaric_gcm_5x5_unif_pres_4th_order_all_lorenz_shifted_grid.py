...
#    Ps_f[:,nphi-1] = Ps_f[:,nphi-2].mean()
    ...
    if Ps_f.min() < 92400.:
        Ps_f[np.where(Ps_f < 92400.)] = 92400.
             ...
             for k in range(nP):
                 ...
                 if k == 0:
                     ...
                     for m in range(0,k):
                         dP_m = dP[0]
                         if j == 0:
                             int_div_v += ((-cosphi[j+1]*-v_n[i-nlambda/2,j+1,k]+8*(--cosphi[j]
                                       *-v_n[i-nlambda/2,j,k]+cosphi[j+1]*v_n[i,j+1,k])
                                       -cosphi[j+2]*v_n[i,j+2,k])/(12*dphi))*dP_m
                         elif j == 1:
                             int_div_v += ((-cosphi[j-1]*-v_n[i-nlambda/2,j-1,k]+8*(-cosphi[j-1]
                                       *v_n[i,j-1,k]+cosphi[j+1]*v_n[i,j+1,k])
                                       -cosphi[j+2]*v_n[i,j+2,k])/(12*dphi))*dP_m
                         elif j == nphi-1:
                             int_div_v += ((cosphi[j-2]*v_n[i,j-2,k]+8*(-cosphi[j-1]
                                       *v_n[i,j-1,k]+-cosphi[j]*-v_n[i-nlambda/2,j,k])
                                       --cosphi[j-1]*-v_n[i-nlambda/2,j-1,k])/(12*dphi))*dP_m
                         elif j == nphi-2:
                             int_div_v += ((cosphi[j-2]*v_n[i,j-2,k]+8*(-cosphi[j-1]
                                       *v_n[i,j-1,k]+cosphi[j+1]*v_n[i,j+1,k])
                                       --cosphi[j+1]*-v_n[i-nlambda/2,j+1,k])/(12*dphi))*dP_m
